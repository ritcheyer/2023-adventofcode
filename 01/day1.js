const input = `rhqrpdxsqhgxzknr2foursnrcfthree
2bmckl
four95qvkvveight5
...
// make an array, split by new lines
const dataArray = input.split('\n');
// const dataArray = testInput.split('\n');

/**
 Function to replace all text-based numbers with their integer equivalents
*/
function findAndReplaceTextNumber(str) {
    let newStr = str
    // replace the overlapping digits
    ...
    // replace the single digit numbers
    .replaceAll('zero', '0')
    .replaceAll('one', '1')
    .replaceAll('two', '2')
    .replaceAll('three', '3')
...
/*
  Given a list of data, find numbers and skip letters.
*/
const findNumber = (dataArray) => {
  
  let thisNumber;
   
  // find the first number
  dataArray.some(letter => {
    
    // if the first character is not a number, return immediately
    const thisCharacter = parseInt(letter);
    if(isNaN(thisCharacter)) {
      return false;
    }
    
    thisNumber = thisCharacter;
    return true;
  })
...
// take in each line of the array and parse for numbers
dataArray.forEach(lineData => {
  // replace all text-based numbers with their integer equivalent
  const newLineData = findAndReplaceTextNumber(lineData);
  // console.log(newLineData);
  
  // split the line into an array.
  const lineArray = newLineData.split('');
    
  // Find the first integer
  const firstNumber = findNumber(lineArray);
  
  // Find the last integer by reversing the line and piping it through the same function
  const lastNumber = findNumber(lineArray.reverse());

  // now that we've found the first and last, create the number
  const thisCalibrationValue = parseInt(`${firstNumber}${lastNumber}`);
  
  value += thisCalibrationValue
});
